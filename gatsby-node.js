...
exports.createPages = ({ graphql, actions }) => {
   ...
   */
  var rawRemoteUrls = JSON.parse(
    fs.readFileSync("content/images/remote_image_urls.json", "utf8"),
  );
  const remoteImages = rawRemoteUrls.map(url => {
    const thumbnailResizeParams =
      "?q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=300&h=300&fit=crop";
  ...
  `).then(result => {
    ...
    console.log("initial image length: ", images.length);
    console.log("initial images: ", images);

    /* Iterate needed pages and create them. */
    const countImagesPerPage = 2;
